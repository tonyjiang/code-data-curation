...
and repository lineage, cleans and splits the organic corpus, runs four generation
...
3. Remove configured evaluation matches, then reserve whole repositories for validation.
...
Let **N** be the number of cleaned organic training documents after the
repository-held-out split. The six SwallowCode, CodeDev, and CodeQA branches each
...
pre-pulled image. See the detailed
[experiment design](docs/codetrace-experiment.md).
...
includes `validation.jsonl`, and the manifest explicitly labels quota shortfalls.
